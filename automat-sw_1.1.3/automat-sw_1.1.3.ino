...
/*
  ...
  V 1.1.3 - Added i2c listener for the extension port by Jelle
...
const int AUTOMAT_NOTEON = 1;
const int AUTOMAT_NOTEOFF = 2;
...
void setOutput(byte i, byte velocity) {
  solenoids.setOutput(i);
  ...
  switch (r) {
    case AUTOMAT_NOTEON:
      if (l = 3) {
        ...
        setOutput(note, velocity);
      }
      break;
    case AUTOMAT_NOTEOFF:
      if (l = 3) {
        char note = Wire.read();
        char velocity = Wire.read();
        clearOutput(note);
