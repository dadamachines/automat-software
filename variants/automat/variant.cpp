...
/*
 * dadamachines automat board variant — pin descriptions
 * Based on Arduino Zero / SAMD21G18A
 * Original source: https://github.com/dadamachines/automat
 *
 * +------------+------------------+--------+-----------------+----------------------------------------
 * + Pin number +  Board pin       |  PIN   | Label/Name      | Comments (* is for default peripheral)
 * +------------+------------------+--------+-----------------+----------------------------------------
 * |            | Digital Low      |        |                 |
 * +------------+------------------+--------+-----------------+----------------------------------------
 * | 0          | 0 -> RX          |  PA11  |                 | *SERCOM0/PAD[3]
 * | 1          | 1 <- TX          |  PA10  |                 | *SERCOM0/PAD[2]
 * | 2          | 2                |  PA14  |                 |
 * | 3          | ~3               |  PA09  |                 | *TCC0/WO[1]
 * | 4          | ~4               |  PA08  |                 | *TCC0/WO[0]
 * | 5          | ~5               |  PA15  |                 | *TC3/WO[1]
 ...
 * | 9          | ~9               |  PA07  |                 | *TCC1/WO[1]
 * | 10         | ~10              |  PA18  |                 | *TC3/WO[0]
 * | 11         | ~11              |  PA16  |                 | *TCC2/WO[0]
 * | 12         | ~12              |  PA19  |                 | *TCC0/WO[3]
 * | 13         | ~13              |  PA17  | LED             | *TCC2/WO[1]
 * +------------+------------------+--------+-----------------+----------------------------------------
 * |            | Analog Connector |        |                 |
 * +------------+------------------+--------+-----------------+----------------------------------------
 * | 14         | A0               |  PA02  | A0              | *ADC/AIN[0]  DAC/VOUT
 * | 15         | A1               |  PB08  | A1              | *ADC/AIN[2]
 * | 16         | A2               |  PB09  | A2              | *ADC/AIN[3]
 * | 17         | A3               |  PA04  | A3              | *ADC/AIN[4]
 * | 18         | A4               |  PA05  | A4              | *ADC/AIN[5]
 * | 19         | A5               |  PB02  | A5              | *ADC/AIN[10]
 * +------------+------------------+--------+-----------------+----------------------------------------
 * |            | Wire             |        |                 |
 * +------------+------------------+--------+-----------------+----------------------------------------
 * | 20         | SDA              |  PA22  | SDA             | *SERCOM3/PAD[0]
 * | 21         | SCL              |  PA23  | SCL             | *SERCOM3/PAD[1]
 * +------------+------------------+--------+-----------------+----------------------------------------
 * |            | SPI (ICSP)       |        |                 |
 * +------------+------------------+--------+-----------------+----------------------------------------
 * | 22         | 1                |  PA12  | MISO            | *SERCOM4/PAD[0]
 * | 23         | 4                |  PB10  | MOSI            | *SERCOM4/PAD[2]
 * | 24         | 3                |  PB11  | SCK             | *SERCOM4/PAD[3]
 * +------------+------------------+--------+-----------------+----------------------------------------
 * |            | LEDs             |        |                 |
 * +------------+------------------+--------+-----------------+----------------------------------------
 * | 25         |                  |  PB03  | RX              |
...
// SERCOM5 is available but not used as a UART in this configuration.
// Serial (USB CDC) is handled by the Adafruit core.
